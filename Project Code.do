...
list security_id issuer rating spread if spread > 900 & numeric_ratings < 12
list security_id issuer rating spread if spread < 0

drop if spread < 0 | (spread > 900 & numeric_ratings < 12)
...
* Save a scatterplot of Y on X with a regression line
twoway (scatter spread numeric_ratings) ///
       (lfit spread numeric_ratings), ///
       ytitle("Yield Spread") xtitle("Credit Rating") legend(off)
...
reg spread numeric_ratings, robust

* Run a regression with maturity only
reg spread maturity, robust

* Run a regression with coupon only
reg spread coupon, robust

* Run a regression with industry dummies only
reg spread i.numeric_industry, robust

* Run a regression with numeric_ratings and maturity
reg spread numeric_ratings maturity, robust

* Run a regression with numeric_ratings and coupon
reg spread numeric_ratings coupon, robust

* Run a regression with numeric_ratings and industry dummies
reg spread numeric_ratings i.numeric_industry, robust

* Run a regression with numeric_ratings, maturity, and coupon
reg spread numeric_ratings maturity coupon, robust

* Run a regression with numeric_ratings, maturity, and industry dummies
reg spread numeric_ratings maturity i.numeric_industry, robust

* Run a regression with numeric_ratings, coupon, and industry dummies
reg spread numeric_ratings coupon i.numeric_industry, robust

* Run a regression with numeric_ratings, maturity, coupon, and industry dummies
reg spread numeric_ratings maturity coupon i.numeric_industry, robust

* * RDD analysis

* Create a centered running variable (distance from cutoff)
gen rating_centered = numeric_ratings - 10.5

* Define treatment variable for ratings 11 and above (junk grade)
gen junk_grade = numeric_ratings >= 11

* Restrict data to a bandwidth around the cutoff without deleting
gen in_bandwidth = (numeric_ratings >= 8 & numeric_ratings <= 13)

* Local linear regression with RDD using only the data within the bandwidth
regress spread rating_centered junk_grade c.rating_centered#c.junk_grade if in_bandwidth

* Visualize the RDD with the restricted data
twoway (scatter spread rating_centered if in_bandwidth) ///
       (lfit spread rating_centered if junk_grade == 0 & in_bandwidth) ///
       (lfit spread rating_centered if junk_grade == 1 & in_bandwidth), ///
       xline(0) xlabel(-2 "BBB" -1 "" 0 "BBB-" 1 "" 2 "BB+") legend(off)
